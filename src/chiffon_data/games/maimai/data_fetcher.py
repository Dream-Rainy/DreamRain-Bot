...
MapSyncCallback = Callable[[dict], Awaitable[object]]


@dataclass
class MaimaiCatalogResult:
    songs: dict[int, MaiSongData]
...
def _ingame_path(context: CatalogContext, sub_dir: str) -> str:
    ...
    return context.ingame_path("maimai", sub_dir)
...
async def _parse_map_xml(
    context: CatalogContext,
    *,
    sync_map_data: MapSyncCallback | None = None,
    sync_map_treasure_data: MapSyncCallback | None = None,
) -> tuple[dict[int, int], dict[int, str], dict[str, int], dict[int, str]]:
    ...
    base = context.ingame_data_base_dir
    if not base:
    ...
    map_dir = _ingame_path(context, "map")
    map_treasure_dir = _ingame_path(context, "mapTreasure")
    map_bonus_music_dir = _ingame_path(context, "mapBonusMusic")
    ...
    if maps:
        ...
        if sync_map_data is not None:
            logger.info("同步Map数据到数据库...")
            await sync_map_data(maps)
    else:
    ...
    if map_treasures:
        ...
        if sync_map_treasure_data is not None:
            await sync_map_treasure_data(map_treasures)
...
async def _attach_lxns_yuzuchan_aliases(
    context: CatalogContext,
    merged_songs: dict[int, MaiSongData],
) -> None:
    ...
    lxns_alias_data = await context.get_json(maimai_alias_list_url(), force_refresh=True)
    ...
    yuzuchan_alias_data = await context.get_json(yuzuchan_maimai_alias_url(), force_refresh=True)
...
async def fetch_maimai_catalog(
    context: CatalogContext,
    *,
    current_data_version: dict[str, Any] | None = None,
    sync_map_data: MapSyncCallback | None = None,
    sync_map_treasure_data: MapSyncCallback | None = None,
) -> MaimaiCatalogResult:
    ...
    music_id_to_map_id, map_id_to_map_name, title_to_music_id, music_id_to_map_name_from_bonus = (
        await _parse_map_xml(
            ...
            sync_map_treasure_data=sync_map_treasure_data,
        )
    )
    ...
    logger.info("=" * 50)
    data_version = await context.get_json(site_config_url(), force_refresh=True)
    if current_data_version and data_version.get("data") == current_data_version.get("data"):
    ...
    dxrating_data = await context.get_json(
    ...
    lxns_song_data = await context.get_json(maimai_song_list_url(notes=True), force_refresh=True)

    # dxrating aliases
    logger.info("获取 dxrating 别名数据...")
    dxrating_aliases_by_song_id: dict[str, list[str]] = {}
    try:
        dxrating_alias_data = await context.get_json(
    ...
    if context.ingame_data_base_dir:
        ...
        logger.info("=" * 50)
        music_dir = _ingame_path(context, "music")
        music_xml_data = scan_music_directory(music_dir)
        if music_xml_data:
            _enrich_with_music_xml(context, merged_songs, music_xml_data)
        else:
            logger.info("未找到Music XML数据")

    # 6. 附加 LXNS + 柚子查别名
    await _attach_lxns_yuzuchan_aliases(context, merged_songs)

    return MaimaiCatalogResult(songs=merged_songs, data_version=data_version)
