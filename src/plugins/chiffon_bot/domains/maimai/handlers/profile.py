...
from ....integrations.lxns.player_api import get_user_data
from ....shared.bot_response import BotResponse
from ..views import mai_bg_draw
from ._timing import format_timing_msg


async def profile(friend_code: str, headers: dict, user_id: str, message_id: int) -> BotResponse:
    logger.debug(f"查询个人资料: friend_code={friend_code}, user={user_id}")
    T = {}
    ...
    if user_data["code"] != 200:
        logger.warning(f"查询个人资料失败: {friend_code} - {user_data['message']}")
