...
from ....integrations.lxns.player_api import get_b50_data, get_user_data
...
async def b50(friend_code: str, headers: dict, user_id: str, message_id: int) -> BotResponse:
    logger.debug(f"查询 B50: friend_code={friend_code}, user={user_id}")
    T = {}
    ...
    b50_data = await get_b50_data(friend_code, headers)
    ...
    if b50_data["code"] != 200:
        logger.warning(f"查询 B50 失败: {friend_code} - {b50_data['message']}")
