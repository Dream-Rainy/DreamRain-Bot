...
async def _fetch_arcade_data_json(gamecode: str) -> dict:
    """从 arcade-songs 的 sites.json 解析 data.json URL 并拉取 payload。"""
    import json

    sites = await http_client.get_json(ARCADE_SITES_JSON_URL, force_refresh=True)
    ...
    payload = await http_client.get_json(data_url, force_refresh=True)
...
def merge_chuni_arcade_and_lxns(arcade_payload: dict, lxns_payload: dict) -> dict[int, ChuniSongData]:
    ...
    for lxns_song in lxns_payload.get("songs", []):
        ...
        if len(arc_candidates) == 1:
            ...
            arc_song = _resolve_arcade_title_conflict(lxns_song, arc_candidates)
...
async def fetch_chunithm_raw_data() -> dict[int, ChuniSongData]:
    ...
    alias_data = await http_client.get_json(chunithm_alias_list_url(), force_refresh=True)
