...
class OauthClient:
    def __init__(self):
        self.client_id = plugin_config.lxns_client_id
        self.client_secret = plugin_config.lxns_client_secret
        self.base_url = getattr(plugin_config, "lxns_base_url", "https://maimai.lxns.net")
        self.redirect_uri = getattr(plugin_config, "lxns_oauth_redirect_uri", "")
        self.scope = getattr(plugin_config, "lxns_oauth_scope", "")
        self.state_ttl_seconds = getattr(plugin_config, "lxns_oauth_state_ttl_seconds", 600)
    ...
    async def get_token(self, auth_code: str, *, code_verifier: str | None = None) -> dict[str, Any]:
        ...
        try:
            result = await http_client.post_json(uri, json_data=data)
