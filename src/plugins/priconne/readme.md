...
- 【缓存运行群】：仅 SUPERUSER 可用，立即将当前监控与历史快照状态写入持久化文件。
- 【提醒掉线】：仅 SUPERUSER 可用，作为自动恢复失败后的人工兜底；只通知未恢复的监控群，并保留其最后一次历史快照。

监控发生临时网络异常或正在重连时会保留恢复信息；如果此时容器或进程重启，机器人启动后仍会尝试自动恢复监控。
