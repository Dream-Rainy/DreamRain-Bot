...
from .. import chara
from .pcrdfans import PCRDSigner
...
def _is_abnormal_ratio(up: int, down: int) -> bool:
    """过滤赞踩比异常的结果（如赞1000+但踩也有几百）"""
    if up >= 1000 and down >= 200:
        return True
    if up >= 500 and up > 0 and down / up > 0.3:
        return True
    return False
...
async def do_query(id_list, region=1, try_cnt=1):
    ...
    if (value - buffer.get(key, 0) < 3600 * 24 * 5) and (exists(_buffer_file(key))):  # 5天内查询过 直接返回
        ...
        if try_cnt == -1:
            ...
            signer = await PCRDSigner.get_instance()
            payload = await signer.get_sign(id_list, region)
            if not payload:
                # 处理异常：签名获取失败
