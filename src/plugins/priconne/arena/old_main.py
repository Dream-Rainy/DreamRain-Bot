...
from ..compat import aiorequests
from ..compat.typing import CQEvent
...
def _load_thumb_icon(name: str, size: int = 16):
    for ext in ("png", "webp", "jpg", "jpeg"):
        ...
        if not exists(path):
            continue
        try:
            return Image.open(path).convert("RGBA").resize((size, size), RESAMPLE_LANCZOS)
        except Exception:
            continue
    return None


def _tint_icon(icon: Image.Image | None, rgb: tuple[int, int, int]) -> Image.Image | None:
    if icon is None:
        return None
    ...
    # 保留透明度，仅对图标内容上色
    return Image.composite(color, Image.new("RGBA", rgba.size, (0, 0, 0, 0)), alpha)
...
async def cut_image(image, hash_size=16):
    ...
    image1 = image.resize((hash_size + 1, hash_size), RESAMPLE_LANCZOS).convert('L')
...
async def calc_distance_arr(arr1, arr2):
    return int(np.abs(arr1 - arr2).sum())
            ...
            if len(border) < 4:  # 由5改为4 是为了日后准备加入残缺队伍查询（通过无api版本）
                ...
                for row_index in range(row_cnt):
                    ...
                    for col_index in range(5):
                        ...
                        cropped = img.crop([x + 2, y + 2, x + w - 2, y + h - 2]).resize((64, 64), RESAMPLE_LANCZOS)
                ...
                outpImg = outpImg.resize((int((outpImg.size)[0] / ratio), int((outpImg.size)[1] / ratio)), RESAMPLE_LANCZOS)
...
async def getUnit(img2):
    img2 = img2.convert("RGB").resize((128, 128), RESAMPLE_LANCZOS)
    img3 = np.array(img2)
...
async def get_pic(address: str):
    resp = await aiorequests.get(address, timeout=6)
...
async def _QueryArenaImageAsync(image_url: str, region: int, bot: HoshinoBot, ev: CQEvent):
    ...
    try:
        ...
        await bot.finish(ev, "图片下载或解析失败，请重试。")
        return
...
async def __arena_query(bot, ev: CQEvent, region: int, defen, raw=0, only_use_cache=False):
    ...
    res = await do_query(defen, region, -1 if only_use_cache else 1)
