...
0表示关闭，1表示{NOTICE_CD_MIN}分钟cd，仅在2点~3点报，
...
@on_command(cmd="竞技场查询").handle()
async def arena_query_handle(
    bot: Bot,
    event: MessageEvent,
    matcher: Matcher,
    ...
    for seg in args:
        if seg.type == "at" and seg.data["qq"] != bot.self_id:
    ...
    # 检查冲突
    if at_users and text_arg and text_arg.isdigit():
        await matcher.finish("@用户 和 UID 不能同时使用")
    ...
    # ---------- #序号模式 ----------
    if text_arg and text_arg.startswith("#"):
        try:
            lid = int(text_arg[1:])
        except ValueError:
            await matcher.finish("序号格式错误")
    ...
    # ---------- UID模式 ----------
    elif text_arg and text_arg.isdigit():
        if len(text_arg) != 13:
            await matcher.finish("位数不对，uid是13位的！")

        pcrid = int(text_arg)
        manual_query_list = [pcrid]
...
""" async def _(
    ...
    for seg in event.get_message():
        if seg.type == "at" and seg.data["qq"] != bot.self_id:
            qid = seg.data["qq"] # at的情况下优先获取at的数据，但不是bot自己的id
    try:
        pcrid = int(group[0])
        if (len(group[0])) != 13:
            await matcher.finish("位数不对，uid是13位的！")
