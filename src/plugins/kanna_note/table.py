...
def Field(**kwargs: Any) -> _FieldInfo:
...
class _PCRDeclarativeBase(DeclarativeBase):
    pass


class PCRModel(_PCRDeclarativeBase):
    __abstract__ = True
    ...
    def __init_subclass__(cls, table: bool = False, **kwargs: Any):
        ...
        for name, annotation in annotations.items():
            ...
            column_type, is_nullable = _unwrap_optional(annotation)
            mapped_annotations[name] = Mapped[column_type]

            if isinstance(value, _FieldInfo):
                field_kwargs = dict(value.kwargs)
                sa_column = field_kwargs.pop("sa_column", None)
                if sa_column is not None:
                    setattr(cls, name, sa_column)
                    continue
                if (
                    is_nullable
                    and "nullable" not in field_kwargs
                    and not field_kwargs.get("primary_key")
                ):
                    field_kwargs["nullable"] = True
                setattr(cls, name, mapped_column(**field_kwargs))
            elif value is ... or value is None:
                setattr(cls, name, mapped_column(nullable=is_nullable))

        cls.__annotations__ = mapped_annotations
        if not table:
