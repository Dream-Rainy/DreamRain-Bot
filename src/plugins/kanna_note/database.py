...
class PCRDatabase:
    ...
    async def init(self):
        if not await self.has_required_tables():
            logger.warning(f"PCR wiki 数据库未初始化或不完整: {self.db_path}，请执行“更新wiki数据库”")
            return
    ...
    async def has_required_tables(self) -> bool:
        async with self.async_session() as session:
            result = await session.execute(
                text(
                    "SELECT name FROM sqlite_master "
                    "WHERE type='table' AND name IN ('unit_data', 'unit_skill_data')"
                )
            )
            existing = {row[0] for row in result.fetchall()}
        return {"unit_data", "unit_skill_data"}.issubset(existing)
